...
Este projeto é um servidor de backend desenvolvido em **Java (Spring Boot)** com interface Web, projetado para interagir com câmeras de controle de acesso e LPR (License Plate Recognition) da **Dahua**.

O sistema atua como um intermediário que recebe notificações de eventos das câmeras e permite o envio de comandos remotos (como abrir portas/cancelas) utilizando autenticação Digest.

## 🚀 Funcionalidades

-   **Monitoramento de Eventos**: Recebe e processa notificações de eventos das câmeras via HTTP multipart.
-   **Comando Remoto**: Envia comandos CGI para as câmeras (ex: abrir porta) suportando métodos GET e POST com autenticação Digest.
